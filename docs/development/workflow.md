...
| D-06 | 기술·DB | Laravel/Python 분리, 복수 DB, Timescale 선택 확장, 단일 스키마 소유권 | 실제 버전·패키지·DB 호환 CI·성능 | 2026-09 기준 안정+장기 패치 가능 조합 제안(착수 시점에 EOL·호환성 재확인 필요): PHP 8.5(Laravel 13 지원 범위, 보안 지원 ~2029-11), Laravel 13(2026-03 출시, 버그 수정 18개월+보안 24개월), Python 3.14(보안 지원 ~2030-10). DB는 PostgreSQL 17 + TimescaleDB 최신 안정(2026-09 기준 TimescaleDB는 PG 16/17만 인증, PG 18 미인증이라 17 선택) 및 MariaDB 12.3 LTS(신규 LTS 정책의 최신 계열; 필드 검증 기간이 짧아 더 보수적 선택이 필요하면 11.8 LTS)를 CI 최소 커버리지로 시험. PHP 8.6(2026-11 GA 예정)·Laravel 14는 정식 출시 후 별도 feature 브랜치에서 검증 | [아키텍처·데이터](architecture.md) |
