...
| D-06 | 기술·DB | Laravel/Python 분리, 복수 DB, Timescale 선택 확장, 단일 스키마 소유권 | 실제 버전·패키지·DB 호환 CI·성능 | 버전 후보는 [아키텍처의 기술 후보 표](architecture.md)를 단일 참조로 사용하며 여기서 중복 기술하지 않는다. 그중 공통 DB의 MariaDB 11.4/11.8 후보는 11.8로 확정 제안: 12.3 LTS는 XML 타입·PL/SQL associative array·TDE passphrase 등 이 프로젝트에 필요 없는 기능뿐이고, `innodb_snapshot_isolation` 기본값 변경으로 REPEATABLE READ 잠금·가시성 동작이 바뀌어 동시 게시 충돌 방지·원자적 포인터 전환 등 트랜잭션 계약을 다시 검증해야 하므로, "동일 기능 계약" 대상인 PostgreSQL과의 검증 부담 대비 얻는 이득이 없다. PostgreSQL 18은 TimescaleDB 2.23.0+에서 지원 확인(2.29.0 기준 16/17/18 지원). CI 매트릭스는 여전히 MariaDB 11.8·PostgreSQL 18 각 1개 + Timescale 확장 조합 1개를 최소 커버리지로 제안하며, 실제 패키지 조합·성능은 착수 시점에 재검증한다 | [아키텍처·데이터](architecture.md) |
