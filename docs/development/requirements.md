...
| FR-003 | 사용자 요청: DNS·ASN 보강 | 제공자·조회 시각·유효기간·실패 기록; 실패가 원본 삭제·안전 판정으로 바뀌지 않음 | [보강](pipeline.md) |
...
| FR-007 | 대화 기준선: 불변 출력 스냅샷 | 미완성·일부 샤드 누락·구세대 덮어쓰기 차단; manifest 고정 다중 파일 일관성 | [API](profiles-api.md), [클러스터](architecture.md) |
...
| FR-009 | 사용자 요청: DNS Resolver 관리·변화 추적 | 시스템/사용자 Resolver 응답 비교, A/AAAA/CNAME·TTL·실패 구분, 추가/제거/재등장 이력 및 파생 출력 | [보강](pipeline.md) |
...
| FR-017 | 사용자 요청: Feed 배포 4개 접근 모드 | 공개/IP 제한/계정 전 IP/계정+IP를 각각 검사; 신뢰 프록시·캐시·이전 revision 우회 불가 | [보안](security-operations.md) |
