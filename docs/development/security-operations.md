...
- 캐시 재사용, 인증 설정 없음, 호출 제한 또는 예약상 건너뜀도 신규 수집 성공과 구분한다. 마지막 시도·최근 검증 성공·현재 제공본의 freshness를 각각 표시한다.
- 빈 결과를 게시할 조건은 D-05에서 확정하며, 전량 파싱 실패를 빈 정상 결과로 처리하지 않는다.
- 필수 산출물 저장·검증·게시 실패를 작업 결과와 호출한 scheduler/CLI/CI의 최종 상태·경보에 전달한다. 오류 후 상태 보고·정리가 실행되더라도 실패를 전체 성공으로 덮어쓰지 않으며, 일부 선택 Source 실패를 허용한 게시도 degraded 사유를 보존한다.
...
지원 소스·출력 형식·실행 방식의 문서 설명은 실제 어댑터 등록·설정 및 계약 시험과 대조한다. 자동 데이터 갱신 커밋이나 CI의 최종 성공 표시만으로 기능 시험 통과·모든 Source의 신규 수집 성공을 판정하지 않는다.
주요 버전·런타임 업그레이드는 별도 feature 브랜치에서 실제 호환을 검증한다.
